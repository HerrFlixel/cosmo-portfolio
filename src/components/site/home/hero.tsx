  ...
  return (
    <section aria-labelledby="hero-title" className="mx-auto max-w-[1400px] px-4 pt-8 md:px-8 md:pt-14">
      <div className="grid items-center gap-12 lg:grid-cols-12 lg:gap-8">
        <h1 id="hero-title" data-intro="headline" data-reveal="lines" className="font-display pb-[0.08em] text-[clamp(2.75rem,7vw,6.75rem)] leading-[0.98] lg:col-span-6">
          {emphasis(headline).map((part, index) =>
            part.italic ? <em key={index}>{part.text}</em> : <Fragment key={index}>{part.text}</Fragment>,
          )}
        </h1>
        {heroes.length > 0 && (
          <div data-intro="collage" className="isolate grid grid-cols-2 gap-4 lg:relative lg:col-span-6 lg:block lg:h-[min(68vh,700px)]">
